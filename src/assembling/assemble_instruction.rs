...
pub fn assemble_instruction(instruction: Instruction, output: &mut IntermediateAssemblingResult) {
    match instruction {
        ...
        Instruction::MovZX(Operand::Register(reg), rm)
            if is_32or64_bit_reg(&reg) =>
        {
            add_rex_opcode_modrm_offset(output, vec![0x0F, 0xB6], rm, RegValue::Register(reg));
        }
        ...
        Instruction::Push(reg) => {
            if let Operand::Register(register) = &reg && !is_64bit_reg(register) {
                panic!("only 64bit register supported for push {register:?}");
            }
